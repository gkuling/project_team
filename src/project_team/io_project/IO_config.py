...
"""
Description: This is an example of a io_config for a machine learning
project. It will hold all necessary parameters on what to load for the
...
io_config_test.save_pretrained(io_config_test.project_folder)
...
class io_config(project_config):
    def __init__(self,
        ...
        None, which becomes "Experiment_on_[current datetime]"
        :param project_folder: path to the location files will be saved for
        the experiment. default: None, which becomes os.getcwd()
        ...
        labels that the output index belong to. Typically a list for
        ...
        be in the range [0.0, 1.0). Use 0.0 (the default) to skip this split
        :param validation_size: portion of data_csv to be used for
        validation. must be in the range [0.0, 1.0). Use 0.0 (the default)
        to skip this split
        ...
        '''
        kwargs.setdefault('config_type', 'IO')
        super(io_config, self).__init__(**kwargs)

        for name, value in (('test_size', test_size),
                            ('validation_size', validation_size)):
            if not 0.0 <= value < 1.0:
                raise ValueError(
                    name + ' must be in the range [0.0, 1.0) — got ' +
                    str(value) +
                    '. Use 0.0 (the default) to skip this split.')

        if experiment_name is None:
            # computed at instantiation time, not import time, so two
            # experiments started in one session get distinct folders
            experiment_name = \
                'Experiment_on_' + dt.now().strftime('%Y-%m-%d_%H-%M-%S')
